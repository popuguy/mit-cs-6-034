...
# family_rules.
male_rule = IF('male (?x)', THEN('male (?x)'))
female_rule = IF('female (?x)', THEN('female (?x)'))
parent_rule = IF('parent (?x) (?y)', THEN('parent (?x) (?y)'))
same_identity_rule = IF(OR('male (?x)', 'female (?x)'), THEN('same-identity (?x) (?x)'))
sibling_rule = IF(AND('parent (?x) (?y)', 'parent (?x) (?z)', NOT('same-identity (?y) (?z)')), THEN('sibling (?y) (?z)', 'sibling (?z) (?y)')) #reflexivity
...
cousin_rule = IF(AND('sibling (?x) (?y)', 'parent (?x) (?c1)', 'parent (?y) (?c2)'), THEN('cousin (?c1) (?c2)', 'cousin (?c2) (?c1)')) #reflexivity
grandparent_rule = IF(AND('parent (?gp) (?p)', 'parent (?p) (?c)'), THEN('grandparent (?gp) (?c)'))
grandchild_rule = IF('grandparent (?x) (?y)', THEN('grandchild (?y) (?x)'))

family_rules = [male_rule, female_rule, parent_rule, same_identity_rule, sibling_rule, brother_rule, sister_rule, mother_rule, father_rule, son_rule, daughter_rule, cousin_rule, grandchild_rule, grandchild_rule]                    # fill me in
...
WHAT_I_FOUND_INTERESTING = 'potential applications to nlp to grasp complex concepts'
WHAT_I_FOUND_BORING = 'tedium of typing out family rules'
